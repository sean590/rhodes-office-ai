...
/**
 * ProviderRecord — the per-provider record body (info, learned routing, and the
 * "Sent" history with revoke). Extracted from the old /service-providers/[id]
 * page so the unified People record page (/people/[id]) renders it, and the old
 * route redirects here. The caller supplies the surrounding wrapper + back link
 * and an onDeleted callback (navigation after delete).
 */
...
interface LearnedRoute { document_type: string; times_confirmed: number; last_sent_at: string | null }
interface ProviderDetail {
  id: string; name: string; disciplines: string[]; domains: string[]; contacts: ProviderContact[];
  default_contact_email: string | null; serves_all_entities: boolean; notes: string | null;
  entity_ids: string[]; learned_routing: LearnedRoute[];
}
interface SendAccess { viewed: number; downloaded: number; last_downloaded_email: string | null; last_access_at: string | null }
interface SendRow {
  id: string; document_id: string; document_name: string | null; document_count: number;
  recipient_email: string; subject: string | null; status: "queued" | "sent" | "failed";
  delivery_provider: string | null; error: string | null; share_token: string | null;
  expires_at: string | null; revoked_at: string | null; sent_at: string | null; created_at: string; access: SendAccess;
}
...
export function ProviderRecord({ providerId, onDeleted }: { providerId: string; onDeleted: () => void }) {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [provRes, sendsRes, entRes] = await Promise.all([
        fetch(`/api/service-providers/${providerId}`),
        fetch(`/api/service-providers/${providerId}/sends`),
        fetch("/api/entities"),
      ]);
  ...
  const activityText = (a: SendAccess) => {
    ...
    if (a.downloaded > 0) parts.push(`Downloaded ${a.downloaded}×${a.last_downloaded_email ? ` by ${a.last_downloaded_email}` : ""}`);
    return parts.join(" · ");
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/service-providers/${providerId}`, { method: "DELETE" });
      if (!res.ok) { const d = await res.json().catch(() => ({})); alert(d.error || "Failed to delete"); setDeleting(false); return; }
      onDeleted();
    } catch { alert("Failed to delete provider"); setDeleting(false); }
  };

  const labelStyle: React.CSSProperties = { fontSize: 11, fontWeight: 600, color: "#9494a0", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 };
  const thStyle: React.CSSProperties = { fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: "#9494a0", padding: "10px 12px", textAlign: "left", borderBottom: "1px solid #e8e6df" };
  const tdStyle: React.CSSProperties = { padding: "12px", borderBottom: "1px solid #f0eee8", fontSize: 13, verticalAlign: "middle" };

  if (loading) return <div style={{ color: "#9494a0", marginTop: 12 }}>Loading…</div>;
  if (!provider) return <div style={{ color: "#9494a0", marginTop: 16 }}>Provider not found.</div>;

  const defaultRecipient = provider.default_contact_email || provider.contacts?.find((c) => c.is_default)?.email || "—";
    ...
    <>
      ...
      <Card style={{ marginBottom: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
          <div><div style={labelStyle}>Domains</div><div style={{ fontSize: 13, color: "#1a1a1f" }}>{(provider.domains ?? []).join(", ") || "—"}</div></div>
          <div><div style={labelStyle}>Default Recipient</div><div style={{ fontSize: 13, color: "#1a1a1f" }}>{defaultRecipient}</div></div>
          <div>
            <div style={labelStyle}>Entities Served</div>
            <div style={{ fontSize: 13, color: "#1a1a1f" }}>
              {provider.serves_all_entities ? "All entities" : provider.entity_ids.length === 0 ? "—" : provider.entity_ids.map((eid) => entityNames[eid] || "—").filter(Boolean).join(", ")}
            </div>
          </div>
          <div>
            <div style={labelStyle}>Contacts</div>
            <div style={{ fontSize: 13, color: "#1a1a1f" }}>
              {(provider.contacts ?? []).length === 0 ? "—" : provider.contacts.map((c) => `${c.name} (${c.email})`).join(", ")}
            </div>
          </div>
        </div>
        {provider.notes && (
          <div style={{ marginTop: 16 }}><div style={labelStyle}>Notes</div><div style={{ fontSize: 13, color: "#1a1a1f", whiteSpace: "pre-wrap" }}>{provider.notes}</div></div>
        )}
      </Card>
        ...
        {provider.learned_routing.length === 0 ? (
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {provider.learned_routing.map((r) => (
              <div key={r.document_type} style={{ border: "1px solid #e8e6df", borderRadius: 8, padding: "8px 12px", background: "#faf9f6" }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#1a1a1f" }}>{humanizeType(r.document_type)}</div>
                <div style={{ fontSize: 11, color: "#9494a0", marginTop: 2 }}>sent {r.times_confirmed}× · last {fmtDate(r.last_sent_at)}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        ...
        {sends.length === 0 ? (
              ...
              return (
                <div key={s.id} style={{ border: "1px solid #f0eee8", borderRadius: 8, padding: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <Link href={`/api/documents/${s.document_id}/download`} style={{ fontSize: 13, fontWeight: 600, color: "#2d5a3d", textDecoration: "none" }}>{docLabel(s)}</Link>
                  ...
                  {ls === "expired" && <div style={{ fontSize: 11, color: "#9494a0", marginTop: 4 }}>Link expired</div>}
                  {s.status === "failed" && s.error && <div style={{ fontSize: 11, color: "#c73e3e", marginTop: 4 }}>{s.error}</div>}
                </div>
              );
            })}
          </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={thStyle}>Document</th><th style={thStyle}>Recipient</th><th style={thStyle}>Status</th>
                <th style={thStyle}>Activity</th><th style={thStyle}>Sent</th><th style={{ ...thStyle, textAlign: "right" }}>Link</th>
              </tr>
            </thead>
            <tbody>
              {sends.map((s) => {
                ...
                return (
                  <tr key={s.id}>
                    <td style={tdStyle}><Link href={`/api/documents/${s.document_id}/download`} style={{ color: "#2d5a3d", fontWeight: 500, textDecoration: "none" }}>{docLabel(s)}</Link></td>
                    <td style={{ ...tdStyle, color: "#6b6b76" }}>{s.recipient_email}</td>
                    <td style={tdStyle}>
                      <Badge label={s.status} color={sc.color} bg={sc.bg} />
                      {s.status === "failed" && s.error && <span style={{ fontSize: 11, color: "#c73e3e", marginLeft: 8 }}>{s.error}</span>}
                    </td>
